    ...
    <div style={{ minHeight:"100vh", background:"#F8F6F1", fontFamily:"'Cormorant Garamond','Georgia',serif", display:"flex", flexDirection:"column", maxWidth:420, margin:"0 auto", position:"relative", overflow:"hidden" }}>
      <style>{`
        ...
        * { box-sizing:border-box; margin:0; padding:0; }
        ...
        .arr-btn { background:none; border:1px solid #E0DCD5; border-radius:50%; width:30px; height:30px; display:flex; align-items:center; justify-content:center; cursor:pointer; transition:border-color 0.15s; flex-shrink:0; }
        .arr-btn:hover { border-color:#1A1A1A; }

        .day-cell { display:flex; flex-direction:column; align-items:center; cursor:pointer; border-radius:6px; padding:3px 2px; transition:background 0.15s; }
        ...
        .detail-overlay { position:absolute; inset:0; background:linear-gradient(to bottom, rgba(0,0,0,0) 0%, rgba(0,0,0,0) 35%, rgba(20,16,12,0.6) 60%, rgba(15,12,8,0.93) 80%, rgba(10,8,5,0.98) 100%); z-index:1; }
        ...
        .back-circle { width:36px; height:36px; border-radius:50%; background:rgba(248,246,241,0.15); border:0.5px solid rgba(248,246,241,0.25); backdrop-filter:blur(8px); display:flex; align-items:center; justify-content:center; cursor:pointer; transition:background 0.15s; }
        .back-circle:hover { background:rgba(248,246,241,0.25); }
        .avatar-circle { width:36px; height:36px; border-radius:50%; background:linear-gradient(135deg,#C8A87A,#8B6840); border:1.5px solid rgba(248,246,241,0.4); display:flex; align-items:center; justify-content:center; font-family:'DM Sans',sans-serif; font-size:13px; font-weight:500; color:#fff; }

        .detail-bottom { position:absolute; bottom:0; left:0; right:0; z-index:3; padding:0 20px 32px; }

        .piece-icon { width:48px; height:52px; border-radius:8px; background:rgba(248,246,241,0.12); border:1px solid rgba(248,246,241,0.18); display:flex; align-items:center; justify-content:center; font-size:22px; cursor:pointer; transition:all 0.15s; backdrop-filter:blur(4px); }
        .piece-icon:hover { background:rgba(248,246,241,0.2); }
        .piece-icon.active { background:rgba(248,246,241,0.88); border-color:#F8F6F1; }
        .piece-lbl { font-family:'DM Sans',sans-serif; font-size:8px; color:rgba(248,246,241,0.5); text-align:center; margin-top:4px; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; max-width:52px; }

        .act-btn { flex:1; padding:12px 8px; border-radius:3px; font-family:'DM Sans',sans-serif; font-size:10px; font-weight:500; letter-spacing:0.08em; text-transform:uppercase; cursor:pointer; transition:all 0.15s; }
        .act-ghost { background:rgba(248,246,241,0.08); border:0.5px solid rgba(248,246,241,0.22); color:rgba(248,246,241,0.65); }
        .act-ghost:hover { background:rgba(248,246,241,0.16); color:#fff; }
        ...
        .swap-item { display:flex; flex-direction:column; align-items:center; gap:4px; cursor:pointer; padding:10px 4px; border-radius:6px; border:1px solid #E0DCD5; transition:all 0.15s; }
        ...
        .swap-confirm { width:100%; padding:14px; background:#1A1A1A; color:#F8F6F1; border:none; border-radius:2px; font-family:'DM Sans',sans-serif; font-size:11px; font-weight:500; letter-spacing:0.1em; text-transform:uppercase; cursor:pointer; margin-top:14px; }
        ...
        .det-btn-primary:hover { background:#333; }

        .plan-btn { width:100%; padding:12px; background:#1A1A1A; color:#F8F6F1; border:none; border-radius:2px; font-family:'DM Sans',sans-serif; font-size:11px; font-weight:500; letter-spacing:0.1em; text-transform:uppercase; cursor:pointer; margin-top:6px; } to{opacity:1;transform:translateY(0);} }
      ...
      <div style={{ flex:1, overflowY:"auto", paddingBottom:100 }}>
        ...
        {/* Detail panel */}
        <div style={{ background:"#fff", borderTop:"0.5px solid #E8E4DC", padding:"14px 20px 20px", margin:"0 0 0 0" }} className="fade-up">
          ...
          {selectedOutfit ? (
            ...
            <>
              ...
              <button className="plan-btn" onClick={planOutfit}>+ Plan an outfit</button>
            </>
          )}
        </div>
      </div>
    ...
    <div style={{ minHeight:"100vh", background:"#F8F6F1", fontFamily:"'Cormorant Garamond','Georgia',serif", display:"flex", flexDirection:"column", maxWidth:420, margin:"0 auto", position:"relative", overflow:"hidden" }}>
      <style>{`
        ...
        * { box-sizing:border-box; margin:0; padding:0; }
        ...
        /* DETAIL VIEW */
        .detail-screen { position:fixed; inset:0; max-width:420px; margin:0 auto; z-index:50; transform:translateX(100%); transition:transform 0.35s cubic-bezier(0.22,1,0.36,1); }
        ...
        .plan-btn { width:100%; padding:12px; background:#1A1A1A; color:#F8F6F1; border:none; border-radius:2px; font-family:'DM Sans',sans-serif; font-size:11px; font-weight:500; letter-spacing:0.1em; text-transform:uppercase; cursor:pointer; margin-top:6px; }
            ...
            <>
              ...
              <div style={{ display:"flex", gap:8, marginBottom:12 }}>
                {selectedOutfit.items.map((item, i) => (
                  <div
                    key={i}
                    onClick={() => { setActivePiece(i); setShowDetail(true); }}
                    style={{ display:"flex", flexDirection:"column", alignItems:"center", gap:4, cursor:"pointer", flex:1 }}
                  >
                    <div style={{ width:"100%", minWidth:44, height:52, background:"#F4F1EC", borderRadius:6, display:"flex", alignItems:"center", justifyContent:"center", fontSize:22, transition:"background 0.15s" }}>{item.e}</div>
                    <span style={{ fontFamily:"'DM Sans',sans-serif", fontSize:9, color:"#AAA", textAlign:"center", lineHeight:1.2 }}>{item.n}</span>
      ...
      {/* ══════════════════════════════════════
          FULL-SCREEN DETAIL POPUP
      ══════════════════════════════════════ */}
      <div className={`detail-screen${showDetail ? " open" : ""}`}>
        ...
        {selectedOutfit && (
          <div className="detail-bottom">
            ...
            <div style={{ display:"flex", gap:8 }}>
              <button className="act-btn act-ghost" onClick={removeOutfit}>Remove</button>
              <button className="act-btn act-ghost" onClick={() => { setSwapChoice(null); setShowSwap(true); }}>Modify</button>
              <button className="act-btn act-solid">Wear this</button>
            </div>
          </div>
        )}
