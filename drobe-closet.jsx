...
import ClosetAnalytics from "./drobe-analytics";
...
export default function DigitalCloset({ onNavigate }) {
  ...
  return (
    <div style={{ minHeight:"100vh", background:"#F8F6F1", fontFamily:"'Cormorant Garamond','Georgia',serif", display:"flex", flexDirection:"column", maxWidth:420, margin:"0 auto" }}>
      ...
      <div style={{ padding:"48px 20px 0", flexShrink:0 }}>
        <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:12 }}>
          ...
          <div style={{ display:"flex", gap:14, alignItems:"center" }}>
            <button style={{ background:"none", border:"none", cursor:"pointer", color:"#1A1A1A", padding:2, display:"flex" }}>
            ...
            <button style={{ background:"none", border:"none", cursor:"pointer", color:"#1A1A1A", padding:2, display:"flex" }} onClick={() => setShowAnalytics(true)} title="Wardrobe Analytics">
              <svg width="18" height="18" fill="none" stroke="currentColor" strokeWidth="1.5" viewBox="0 0 24 24"><path d="M18 20V10M12 20V4M6 20v-6" strokeLinecap="round" strokeLinejoin="round"/></svg>
            </button>
          </div>
        </div>
      ...
      {showAnalytics && (
        <ClosetAnalytics onClose={() => setShowAnalytics(false)} />
      )}
    </div>
  );
}
